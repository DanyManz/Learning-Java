public class Circle implements Main.Shape {
	...
	*/
    ...
    public static Circle fromPoints(Point p1, Point p2, Point p3) {
        ...
        double newCenterX = ((x1 * x1 + y1 * y1) * (y2 - y3) + (x2 * x2 + y2 * y2) * (y3 - y1) + (x3 * x3 + y3 * y3) * (y1 - y2)) / D;
        double newCenterY = ((x1 * x1 + y1 * y1) * (x3 - x2) + (x2 * x2 + y2 * y2) * (x1 - x3) + (x3 * x3 + y3 * y3) * (x2 - x1)) / D;
        
        Point newCircleCenter = new Point(newCenterX, newCenterY);
        double newCircleRadius = Point.distance(newCircleCenter, p1);

        return new Circle(newCircleCenter, newCircleRadius);
